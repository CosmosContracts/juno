...
command -v junod >/dev/null 2>&1 || {
  echo >&2 "junod command not found. Ensure this is setup / properly installed in your GOPATH."
  exit 1
}
...
from_scratch() {
  ...
  update_test_genesis() {
    ...
    cmd <"$HOME"/.juno/config/genesis.json | jq "$1" >"$HOME"/.juno/config/tmp_genesis.json && mv "$HOME"/.juno/config/tmp_genesis.json "$HOME"/.juno/config/genesis.json
  }
