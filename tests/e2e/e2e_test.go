...
func (s *IntegrationTestSuite) TestTokenFactoryBindings() {
	...
	stdout := chainA.NodeConfigs[0].StoreWasmCode("scripts/token_reflect.wasm", addr)
	fmt.Println(stdout)
	...
	chainA.NodeConfigs[0].WasmExecute(cAddr, `{"reflect":{"amount":"1000000","denom":"ujuno"}}`, addr)
}
