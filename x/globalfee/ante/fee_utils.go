...
func DenomsSubsetOfIncludingZero(coins, coinsB sdk.Coins) bool {
	...
	for _, coin := range coins {
		...
		if !coinsB.DenomsSubsetOf(sdk.NewCoins(coin)) {
			return false
		}
	}
...
func IsAnyGTEIncludingZero(coins, coinsB sdk.Coins) bool {
	...
	for _, coin := range coins {
		...
		if ok := coinsB.DenomsSubsetOf(sdk.NewCoins(coin)); ok {
...
func CombinedFeeRequirement(globalFees, minGasPrices sdk.Coins) sdk.Coins {
	...
	for _, fee := range globalFees {
		...
		ok := minGasPrices.DenomsSubsetOf(sdk.NewCoins(fee))
		if ok {
			c := minGasPrices.AmountOf(fee.Denom)
			if c.GT(fee.Amount) {
				allFees = append(allFees, sdk.NewCoin(fee.Denom, c))
			} else {
