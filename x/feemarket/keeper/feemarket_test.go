...
func (s *KeeperTestSuite) TestUpdateFeeMarket() {
	s.Run("empty block with default eip1559 with min base fee", func() {
		...
		s.Require().NoError(s.App.AppKeepers.FeeMarketKeeper.UpdateFeeMarket(s.Ctx))

		fee, err := s.App.AppKeepers.FeeMarketKeeper.GetBaseGasPrice(s.Ctx)
		s.Require().NoError(err)
		s.Require().Equal(fee, params.MinBaseGasPrice)

		lr, err := s.App.AppKeepers.FeeMarketKeeper.GetLearningRate(s.Ctx)
		s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestGetMinGasPrices() {
	s.Run("can retrieve min gas prices with default eip-1559", func() {
		...
		mgp, err := s.App.AppKeepers.FeeMarketKeeper.GetCurrentGasPrices(s.Ctx)
		s.Require().NoError(err)
