...
func TestValidatePriceTrackingList(t *testing.T) {
	err := validatePriceTrackingList("invalidUint64")
	require.ErrorContains(t, err, "invalid parameter type: string")

	err = validatePriceTrackingList(DenomList{
		{BaseDenom: ""},
	})
	require.ErrorContains(t, err, "oracle parameter PriceTrackingList Denom must have BaseDenom")

	err = validatePriceTrackingList(DenomList{
		{BaseDenom: DenomJuno.BaseDenom, SymbolDenom: ""},
	})
	require.ErrorContains(t, err, "oracle parameter PriceTrackingList Denom must have SymbolDenom")
...
func TestParamsEqual(t *testing.T) {
	...
	// empty name
	p9 := DefaultParams()
	p9.PriceTrackingList[0].BaseDenom = ""
	p9.PriceTrackingList[0].SymbolDenom = "ATOM"
	err = p9.Validate()
	require.Error(t, err)

	// empty
	p10 := DefaultParams()
	p10.PriceTrackingList[0].BaseDenom = "uatom"
	p10.PriceTrackingList[0].SymbolDenom = ""
	err = p10.Validate()
	require.Error(t, err)
