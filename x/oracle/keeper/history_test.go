...
func TestStoreAndGetMultipleHistoricalData(t *testing.T) {
	...
	for _, tc := range []struct {
		denom     string
		phEntries []types.PriceHistoryEntry
	}{
		{
			denom: "Denom",
			phEntries: []types.PriceHistoryEntry{
		...
		for _, phEntry := range tc.phEntries {
			oracleKeeper.storeHistoricalData(ctx, tc.denom, phEntry)
		}

		phStoreDenom, err := oracleKeeper.getHistoryEntryAtOrBeforeTime(ctx, tc.denom, tc.phEntries[0].PriceUpdateTime.Add(-time.Minute))
		require.Error(t, err)
		require.Equal(t, types.PriceHistoryEntry{}, phStoreDenom)
		phStoreDenom, err = oracleKeeper.getHistoryEntryAtOrBeforeTime(ctx, tc.denom, tc.phEntries[0].PriceUpdateTime)
		require.NoError(t, err)
		require.Equal(t, tc.phEntries[0], phStoreDenom)
		phStoreDenom, err = oracleKeeper.getHistoryEntryAtOrBeforeTime(ctx, tc.denom, tc.phEntries[0].PriceUpdateTime.Add(time.Minute))
		require.NoError(t, err)
		require.Equal(t, tc.phEntries[0], phStoreDenom)
		phStoreDenom, err = oracleKeeper.getHistoryEntryAtOrBeforeTime(ctx, tc.denom, tc.phEntries[1].PriceUpdateTime)
		require.NoError(t, err)
		require.Equal(t, tc.phEntries[1], phStoreDenom)
		phStoreDenom, err = oracleKeeper.getHistoryEntryAtOrBeforeTime(ctx, tc.denom, tc.phEntries[1].PriceUpdateTime.Add(time.Minute))
		require.NoError(t, err)
		require.Equal(t, tc.phEntries[1], phStoreDenom)
		...
		phStoresDenom, err := oracleKeeper.getHistoryEntryBetweenTime(
			ctx,
			tc.denom,
			tc.phEntries[0].PriceUpdateTime.Add(-time.Minute),
			tc.phEntries[2].PriceUpdateTime.Add(time.Minute),
		)
		require.NoError(t, err)
		require.Equal(t, phStoresDenom, tc.phEntries)

		phStoresDenom, err = oracleKeeper.getHistoryEntryBetweenTime(
			ctx,
			tc.denom,
			tc.phEntries[0].PriceUpdateTime.Add(-time.Minute),
			tc.phEntries[1].PriceUpdateTime,
		)
		require.NoError(t, err)
		require.Equal(t, 2, len(phStoresDenom))
		require.Equal(t, phStoresDenom[0], tc.phEntries[0])
		require.Equal(t, phStoresDenom[1], tc.phEntries[1])
	}
}
